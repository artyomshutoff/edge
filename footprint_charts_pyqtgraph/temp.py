class MarketProfileItem(QGraphicsItem):
    def __init__(self, **kwargs):
        ...
        self.interval = np.array(kwargs["interval"], dtype=np.float64)
        self.period = np.array(kwargs["period"], dtype=np.float64)
    ...
    def generate_values(self):
        if self.open_close_show:
            ...
            for i in range(len(self.period)):
                o = format_value(self.period[i][1], self.tick_size)
                c = format_value(self.period[i][4], self.tick_size)
                self.period_open_close.append([o, c])

        self.period = list(map(lambda x: x / 1000, self.period[:, 0]))
        diff = self.period[1] - self.period[0]
        diff2 = self.interval[:, 0][1] - self.interval[:, 0][0]
    ...
    def generate_picture(self):
        ...
        w = (self.interval[:, 0][1] - self.interval[:, 0][0]) / 3000
